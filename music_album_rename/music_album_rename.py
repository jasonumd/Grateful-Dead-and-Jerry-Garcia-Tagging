...
db = "JerryBase.db"
mics = ["nak","sony","schoeps","mk41","m88","mk4","km84i","km184","km140","ccm","cmc","akg","senn","mg210","at4053","re20","cemc","ck41"]

#Log Files
nonflac_log = os.path.join(os.getcwd(), "logs", "nonflac_log.txt")
early_and_late_log = os.path.join(os.getcwd(), "logs", "early_and_late_log.txt")
early_log = os.path.join(os.getcwd(), "logs", "early_log.txt")
late_log = os.path.join(os.getcwd(), "logs", "late_log.txt")
date_not_found_log = os.path.join(os.getcwd(), "logs", "date_not_found_log.txt")
no_version_log = os.path.join(os.getcwd(), "logs", "no_version_log.txt")
no_shnid_log = os.path.join(os.getcwd(), "logs", "no_shnid_log.txt")
multiple_records = os.path.join(os.getcwd(), "logs", "multiple_records.txt")
skipped_folder = os.path.join(os.getcwd(), "logs", "skipped_folder.txt")
...
for root, dirs, files in os.walk(path):
    ...
    #Attempt to discern if this show is a sounboard, audience recording, etc.
    Version = ""

    if "mtx" in Folder.lower():
        Version = " (mtx"
    elif "sbd" in Folder.lower():
        Version = " (sbd"
    elif "aud" in Folder.lower():
        Version = " (aud"
    elif any(mic in Folder.lower() for mic in mics):
        Version = " (aud"
        ...
        Version = " (pa"
    elif "5-1" in Folder.lower():
        Version = " (5.1"
    elif "dts" in Folder.lower():
        Version = " (dts"
    else:
